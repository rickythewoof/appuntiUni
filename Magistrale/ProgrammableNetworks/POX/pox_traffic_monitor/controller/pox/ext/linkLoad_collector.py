...
class trafficMeter():
	...
	def _handle_ConnectionUp (self, event):
		if event.ofp.in_port == self.dpid:
	...
	def _handle_FlowStatsReceived(self, event):
		...
		for f in event.stats:
			if f.match.nw_dst == self.dst and f.match.nw_src == self.src:
	...
	def send_stat_req(self):
		self.conection.send(of.ofp_stats_request(body=of.ofp_flow_stats_request()))
