...
def equivalenti(out1,out2,comparison,error):
    if comparison == None:
        if error == 0:
            return out1 == out2
        else:
            return abs(out1-out2) < error
    if error == 0:
        return comparison(out1,out2)
    else:
        return comparison(out1,out2,error)

def tester_fun_comp(function, input_data, output_data,compare = None, error = 0):
    ...
    if len(input_data)==0 or type(output_data) == type(None):
...
#        sys.stdout = stdout    # decommentare nel caso si disattivino le print
        ...
        if type(result_container[0]) == type(None):
            print("-"* 34 + "\n")
        ...
        # in caso di eccezione
            ...
            if not equivalenti(output_data,result_container[0],compare,error):
                print('Risultato Test: NEGATIVO'+'\n\n'+'*'*30+'\n')
                return 0
            else:
